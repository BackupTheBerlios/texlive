/*  $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/texlive/Repository/source.development/TeX/texk/dvipdfmx/src/pdfencrypt.c,v 1.1 2004/01/08 13:44:14 rahtz Exp $
...
static void compute_id_string (char *dviname, char *pdfname)
{
  ...
  MD5_Update(&md5_ctx, date_string, strlen(date_string));
  RELEASE (date_string);

  producer = NEW (strlen(PRODUCER)+strlen(PACKAGE)+strlen(VERSION), char);
  sprintf(producer, PRODUCER, PACKAGE, VERSION);
  MD5_Update(&md5_ctx, producer, strlen(producer));
  RELEASE (producer);

  MD5_Update(&md5_ctx, dviname, strlen(dviname));
  MD5_Update(&md5_ctx, pdfname, strlen(pdfname));
...
static void compute_owner_password (void)
{
   ...
   */
  MD5_Init(&md5_ctx);
  MD5_Update(&md5_ctx, in_buf, MAX_STR_LEN);
  ...
  if (revision == 3)
    for (i = 0; i < 50; i++) {
      MD5_Init(&md5_ctx);
      MD5_Update(&md5_ctx, md5_buf, MAX_KEY_LEN);
      MD5_Final(md5_buf, &md5_ctx);
    }
  /*
   ...
   *    Length entry.
   */
  RC4_set_key(&key, key_size, md5_buf);
  /*
   ...
   */
  RC4(&key, MAX_STR_LEN, in_buf, out_buf);
  /*
  ...
  if (revision == 3)
    for (i = 1; i <= 19; i++) {
      ...
      RC4_set_key(&key, key_size, key_buf);
      RC4(&key, MAX_STR_LEN, in_buf, out_buf);
    }
  /*
...
static void compute_encryption_key (unsigned char *pwd)
{
   ...
   *    computed.)
   */
  MD5_Update(&md5_ctx, opwd_string, MAX_STR_LEN);
  /*
   ...
   *    finish the hash.
   */
  MD5_Update(&md5_ctx, id_string, MAX_KEY_LEN);
  MD5_Final(md5_buf, &md5_ctx);
  /*
...
static void compute_user_password (void)
{
  ...
  switch (revision) {
  case 2:
    RC4_set_key(&key, key_size, key_data);
    RC4(&key, MAX_STR_LEN, padding_string, out_buf);
    break;
  case 3:
    MD5_Init(&md5_ctx);
    MD5_Update(&md5_ctx, padding_string, MAX_STR_LEN);

    MD5_Update(&md5_ctx, id_string, MAX_KEY_LEN);
    MD5_Final(md5_buf, &md5_ctx);

    RC4_set_key(&key, key_size, key_data);
    RC4(&key, MAX_KEY_LEN, md5_buf, out_buf);
    ...
    for (i = 1; i <= 19; i++) {
      ...
      RC4_set_key(&key, key_size, key_buf);
      RC4(&key, MAX_KEY_LEN, in_buf, out_buf);
    }
    break;
  default:
...
void pdf_encrypt_data (unsigned char *data, unsigned long len)
{
  ...
  MD5_Init(&md5_ctx);
  MD5_Update(&md5_ctx, in_buf, key_size+5);
  MD5_Final(md5_buf, &md5_ctx);
  
  result = NEW (len, unsigned char);
  RC4_set_key(&key, (key_size > 10 ? MAX_KEY_LEN : key_size+5), md5_buf);
  RC4(&key, len, data, result);

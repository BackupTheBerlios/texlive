/*  $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/texlive/Repository/source.development/TeX/texk/dvipdfmx/src/encodings.c,v 1.1 2004/01/08 13:44:14 rahtz Exp $
...
int get_encoding (const char *enc_name)
{
  ...
  strcat (tmp, ".enc");
#ifdef MIKTEX
  full_enc_filename = work_buffer;
  if (!miktex_find_psheader_file(enc_name, full_enc_filename) &&
      !miktex_find_app_input_file("dvipdfm", enc_name, full_enc_filename) &&
      !miktex_find_psheader_file(tmp, full_enc_filename) &&
      !miktex_find_app_input_file("dvipdfm", tmp, full_enc_filename)) {

/*  $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/texlive/Repository/source.development/TeX/texk/dvipdfmx/src/pdfspecial.c,v 1.1 2004/01/08 13:44:15 rahtz Exp $
...
static void do_bxform (char **start, char *end, double x_user, double y_user)
{
  ...
  if (parse_dimension(start, end, p)) {
    ...
    if (p->width != 0.0 || p->height != 0.0 || p->depth != 0.0)
      fprintf(stderr, "[special] bxform: width, height, and depth are meaningless. These will be ignored.\n");
    if (p->scale != 0.0) {
      p->xscale = p->scale;
      p->yscale = p->scale;
    }
...
{
  skip_white(start, end);
  set_tounicode_cmap(parse_ident(start, end));
}
  ...
  if (result) { /* Put reference to object on page */
    ...
    if (dev_wmode() == 1) { /* rotated 90 deg. in vertical writing mode */
      add_xform_matrix (x_user, y_user,
			p->xscale/pdf_dev_scale(), p->yscale/pdf_dev_scale(), p->rotate - 0.50 * M_PI);
    } else {
      add_xform_matrix (x_user, y_user,
...
static void do_obj (char **start, char *end)
{
  ...
  if ((name = parse_opt_ident(start, end))) {
    if ((result = parse_pdf_object(start, end)))
      ...
      fprintf(stderr, "Special object: Could not find an object for (%s).\n", name);
    RELEASE (name);
    return;
  }
  fprintf(stderr, "Special object: Could not find a reference name.\n");
}
			  ...
			  char *res_name)
{
  ...
  { /* Compute new xscale and yscale based on user specified values
       ...
       images to 1x1 :-( */
    int width, height;
    width = (int)pdf_number_value(pdf_lookup_dict (image_dict, "Width"));
    height = (int)pdf_number_value(pdf_lookup_dict (image_dict, "Height"));

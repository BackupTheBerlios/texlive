...
/********************************************************/
/* t4ht.c                              2003-06-16-01:33 */
/* Copyright (C) 1998--2003  Eitan M. Gurari            */
/*                                                      */
...
#ifndef KPATHSEA
#ifdef PLATFORM
   (IGNORED) printf("t4ht.c (2003-06-16-01:33 %s)\n",PLATFORM);
#else
   (IGNORED) printf("t4ht.c (2003-06-16-01:33)\n");
#endif
#else
#ifdef PLATFORM
   (IGNORED) printf("t4ht.c (2003-06-16-01:33 %s kpathsea)\n",PLATFORM);
#else
   (IGNORED) printf("t4ht.c (2003-06-16-01:33 kpathsea)\n");
#endif
#endif
...
if( rec_op != No_op ){
  addr = 0; while( (*ch>='0') && (*ch<='9') ){
    addr = addr*10 + *ch - '0'; ch++;
         ...
         if( rec_op == No_op ){
            (IGNORED) fprintf(stderr,warning);
            (IGNORED) fprintf(stderr,"CopyTo: %s%s%s%s?\n",
                      match[1], match[2], match[3], match[4], match[5]);
         } else {

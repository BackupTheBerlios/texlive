...
/********************************************************/
/* tex4ht.c                            2003-06-16-01:57 */
/* Copyright (C) 1996--2003     Eitan M. Gurari         */
/*                                                      */
...
"   [-c<tag name>]  choose in env files\n"
"   [-e<env-file>]\n"
"   [-g<bitmap-file-ext>]\n"
...
next_str
   ...
   print_f(next_str); free((void *) next_str);
   next_str = (char *) 0;
} else {
   if( verb_ch ){
...
#ifndef KPATHSEA
#ifdef PLATFORM
   (IGNORED) printf("tex4ht.c (2003-06-16-01:57 %s)\n",PLATFORM);
#else
   (IGNORED) printf("tex4ht.c (2003-06-16-01:57)\n");
#endif
#else
#ifdef PLATFORM
   (IGNORED) printf("tex4ht.c (2003-06-16-01:57 %s kpathsea)\n",PLATFORM);
#else
   (IGNORED) printf("tex4ht.c (2003-06-16-01:57 kpathsea)\n");
#endif
#endif
for(i=0; i<argc; i++){
...
if( !dot_file ){
   ...
   if( !dot_file ){
      if( dump_env_search ){ (IGNORED) printf("%s?\n", ".tex4ht"); }
      if( dot_file = f_open(".tex4ht", READ_TEXT_FLAGS) ){
         printf("(%s)\n", ".tex4ht");
   }  }
#endif
...
envname= kpse_find_file ("tex4ht.env", kpse_program_text_format, 0);
  ...
  if ( envname ){
    ...
    if( p = (char *) kpse_var_value( "TEX4HTINPUTS" ) ){
...
{                    U_CHAR * q;
  if( q = (U_CHAR *) kpse_var_value( "TEX4HTFONTSET" ) ){
    if( (int) strlen(q) > 0 ){
        export_str = (char *) r_alloc((void *) export_str,
...
if( p = (U_CHAR *) kpse_var_value( "TEX4HTFONTSET" ) ){

...
sub locateMap {
  ...
  @files = `kpsewhich --format="dvips config" @files`;
  if (wantarray) {
    return @files;
  }
  else {
...
sub setOption {
  ...
  if ($opt eq "LW35"
      and $val !~ m/^(URWkb|URW|ADOBE|ADOBEkb)$/) {
...
sub dvips2dvipdfm {
  ...
  map {
    ...
    $_ =~ s@(.*<<*\s*([^ ]*)\.pf[ab].*)@\1 \2@x;
    if (m/%[^ ]* [^ ]*$/x) { $_ =~ s@( ([^ ]*).*)$@\1 \2@; }
    $_ =~ s@(.* ([.0-9-][.0-9-]*) *ExtendFont.*)@\1 -e \2@x;
    $_ =~ s@(.* ([.0-9-][.0-9-]*) *SlantFont.*)@\1 -s \2@x;
    $_ =~ s@.*%@@ ;
  } @lines;
